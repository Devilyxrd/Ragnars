...
$qry = $db->query("SELECT * FROM `219f2_banlist` WHERE hash = '$hash'");
...
if ($count != 1 || $acccess < 1) {
    $accessLogger = $db->query("INSERT INTO 219f2_logs (message, type, date) VALUES ('Başarısız giriş denemesi. Engellenen IP adresi: $ip $name $surname $email', 'Banlı Hesap', '$login_time')");
    ...
    Yasaklı Kullanıcı erişimi tespit edildi. Lütfen doğru izinlere sahip olduğunuzdan emin olun.
